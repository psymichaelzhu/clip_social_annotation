...
def annotate_one_dimension(dimension_embedding_df, video_embedding_df):
    """
    Calculate scores for each video to be classified into one of the labels within this dimension
...
def annotate_all_dimensions(annotation_dimension_dict, video_embedding_df):
    ...
    Returns:
        results(dict): Dictionary with dimension names as keys and DataFrames as values, rows are videos, columns are labels, containing normalized similarity scores
    """ 
    results = {}
...
def visualize_annotation_distributions(annotation_df, color_map=plt.cm.rainbow,video_dir='../data/MiT_original_videos'):
    ...
    Args:
        annotation_df(DataFrame): DataFrame containing annotation scores for each label, 
                                     with video_name as first column
    """
    # Get label columns (all columns except video_name)
    labels = [col for col in annotation_df.columns if col != 'video_name']
    n_labels = len(labels)
    ...
    colors = color_map(np.linspace(0, 1, n_labels))

    for i, label in enumerate(labels):
        ax = axes[i] if n_labels > 1 else axes[0]
        # Plot histogram in main subplot with color from colormap
        ax.hist(annotation_df[label], bins=20, density=True, alpha=0.7, color=colors[i])
        ...
        quantile_scores = np.quantile(annotation_df[label], quantiles)
        
        # Find nearest videos to each quantile
        example_videos = []
        for q_score in quantile_scores:
            nearest_idx = (annotation_df[label] - q_score).abs().idxmin()
        ...
        for video in example_videos:
            ...
            score = annotation_df[annotation_df['video_name'] == video][label].iloc[0]
...
def show_extreme_videos(annotation_df, label, video_dir='../data/MiT_original_videos'):
    """
    Show top 5 and bottom 5 videos for a given annotation dimension
    
    Args:
        annotation_df(DataFrame): DataFrame containing annotation scores for each label, with video_name as first column
        label(str): Column name of the label to analyze
    ...
    sorted_df = annotation_df.sort_values(by=label)
    ...
    plt.suptitle(f'{label}', fontsize=30)  # Increased suptitle font size
    plt.tight_layout()
    plt.show()

def visualize_dimension_embeddings(dimension_dict):
    """
    Visualize embeddings of dimension names and their labels in UMAP space
    
    Args:
        dimension_dict (dict): Dictionary mapping dimension names to lists of labels
    ...
    # Get all words and their types (dimension vs label)
    all_words = []
    word_types = []  # 'dimension' or 'label' 
    dimension_ids = []  # which dimension each word belongs to
    
    for i, (dimension, labels) in enumerate(dimension_dict.items()):
        ...
        for label in labels:
            ...
            word_types.append('label')
            dimension_ids.append(i)
    
    # Get CLIP embeddings for all words
    with torch.no_grad(), torch.autocast("cuda"):
        text = tokenizer(all_words)
    ...
    plt.title('Embeddings for Dimension and Label', fontsize=26)  # Increased title font size
...
def show_similar_videos_by_dimension(video_name, dimension_name, annotation_results, n_videos=5):
    """
    Examine videos most similar and dissimilar to a target video along a specific dimension
    
    Args:
        video_name(str): Name of the target video
        dimension_name(str): Name of the dimension to compare on
        annotation_results(dict): Dictionary mapping dimensions to annotation dataframes
        n_videos(int): Number of most/least similar videos to show
        
    Returns:
        None: Displays frames and similarity scores
    """
    # Get annotation dataframe for this dimension
    dim_df = annotation_results[dimension_name]
    
    # Get target video's label scores
    target_scores = dim_df[dim_df['video_name'] == video_name]
    
    # Calculate cosine similarity between target and all other videos
    other_videos = dim_df[dim_df['video_name'] != video_name]
    similarities = dict(zip(
        other_videos['video_name'],
        cosine_similarity(target_scores.iloc[:,1:], other_videos.iloc[:,1:])[0]
    ))
    
    # Sort videos by similarity
    sorted_videos = sorted(similarities.items(), key=lambda x: x[1], reverse=True)
    
    # Display target video
    print(f"\nTarget video: {video_name}")
    target_frame = extract_frame(video_name)
    plt.figure(figsize=(5,5))
    plt.imshow(target_frame)
    plt.axis('off')
    plt.title(f"Target: {video_name}", fontsize=22)
    plt.show()
    
    # Display most similar videos
    print(f"\nMost similar videos on dimension '{dimension_name}':")
    plt.figure(figsize=(20,4))
    for i, (vid, sim) in enumerate(sorted_videos[:n_videos]):
        frame = extract_frame(vid)
        plt.subplot(1,n_videos,i+1)
        plt.imshow(frame)
        plt.axis('off')
        plt.title(f"{vid}\nsim={sim:.3f}", fontsize=18)
    plt.tight_layout()
    plt.show()
    
    # Display least similar videos  
    print(f"\nLeast similar videos on dimension '{dimension_name}':")
    plt.figure(figsize=(20,4))
    for i, (vid, sim) in enumerate(sorted_videos[-n_videos:]):
        frame = extract_frame(vid)
        plt.subplot(1,n_videos,i+1)
...
# %% annotation matches intuition
target_dimension_list = ['valence', 'relationship']
for target_dimension in target_dimension_list:
    visualize_annotation_distributions(annotation_results[target_dimension])
    for label in dimension_dict[target_dimension]:
        show_extreme_videos(annotation_results[target_dimension], label)


# %% annotation captures similarity
# randomly select 5 videos
np.random.seed(42)
video_name_list = np.random.choice(video_name_list, 5, replace=False)
for video_name in video_name_list:
    show_similar_videos_by_dimension(video_name, 'relationship', annotation_results)
    show_similar_videos_by_dimension(video_name, 'activity', annotation_results)
